...
-- Migration: 20251219_create_ai_chat_support.sql
--
-- Creates:
...
CREATE TABLE IF NOT EXISTS public.faq_knowledge_base (
  ...
  -- Multi-tenant support (nullable, FK added later if service_businesses exists)
  business_id BIGINT,
...
-- Update response variables for FAQs
...
WHERE id IN (SELECT id FROM public.faq_knowledge_base LIMIT 5);

-- -----------------------------------------------------------------------------
-- Add Foreign Key Constraints (if service_businesses table exists)
-- -----------------------------------------------------------------------------

-- Check if service_businesses table exists and add FK constraints
DO $$
BEGIN
  IF EXISTS (
    ...
    -- Add foreign key constraint to faq_knowledge_base
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.table_constraints 
      WHERE constraint_name = 'faq_knowledge_base_business_id_fkey'
    ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.table_constraints 
      WHERE constraint_name = 'chat_conversations_business_id_fkey'
    ) THEN
      ALTER TABLE public.chat_conversations
      ADD CONSTRAINT chat_conversations_business_id_fkey
      FOREIGN KEY (business_id) REFERENCES public.service_businesses(id) ON DELETE CASCADE;
    END IF;
  END IF;
END $$;
